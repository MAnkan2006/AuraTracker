...
const Attendance = () => {
  ...
  const handleMark = (status) => {
    ...
    markAttendance(selectedSubject, today, status);
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      days.push(
        <div 
          ...
          onClick={() => {
            if (!selectedSubject) {
              setAlertMessage("Please select a subject first!");
              setIsAlertOpen(true);
              return;
            }
          ...
          className={`p-2 md:p-4 rounded-xl border flex items-center justify-center transition-all hover:scale-105 cursor-pointer ${statusClass} ${isToday ? 'ring-2 ring-[var(--accent)]' : ''}`}
        >
          {i}
        </div>
      );
  ...
  return (
    <div className="space-y-6 pb-20 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className={glassPanelClass}>
          <div className="text-sm font-bold text-[var(--text-secondary)] group-data-[scheme=light]:text-gray-500 uppercase tracking-wider mb-2">Attendance Rate</div>
          <div className="text-3xl font-extrabold text-[var(--text-primary)] group-data-[scheme=light]:text-gray-900 mb-4">{stats.percentage}%</div>
          <div className="w-full bg-white/10 group-data-[scheme=light]:bg-gray-200 rounded-full h-2 shadow-inner">
            <div className="bg-gradient-to-r from-blue-500 to-indigo-600 h-2 rounded-full shadow-[0_0_10px_rgba(59,130,246,0.5)]" style={{ width: `${stats.percentage}%` }}></div>
          </div>
        </div>
        
        <div className={glassPanelClass}>
          <div className="text-sm font-bold text-[var(--text-secondary)] group-data-[scheme=light]:text-gray-500 uppercase tracking-wider mb-2">Current Streak</div>
          <div className="text-3xl font-extrabold text-[var(--text-primary)] group-data-[scheme=light]:text-gray-900 mb-1">12 days</div>
          <div className="text-sm font-medium text-[var(--text-muted)] group-data-[scheme=light]:text-gray-500">Consecutive presents</div>
        </div>
        ...
        <div className={glassPanelClass}>
          ...
          <div className="text-xl font-bold text-[var(--text-primary)] group-data-[scheme=light]:text-gray-900 mb-2">Mon, Wed, Fri</div>
          <div className="text-sm font-medium text-[var(--text-muted)] group-data-[scheme=light]:text-gray-500">Total Logs: {stats.total}</div>
        </div>
      </div>
      ...
      <div className={`${glassPanelClass} overflow-hidden`}>
        ...
        <div className="flex items-center gap-2 px-4 py-3 mb-6 bg-[var(--accent)]/10 text-[var(--accent)] group-data-[scheme=light]:bg-blue-50 group-data-[scheme=light]:text-blue-700 rounded-xl border border-[var(--accent)]/20 text-sm font-medium">
          <Info size={18} />
          <span>Logging active only on scheduled class days.</span>
        </div>
